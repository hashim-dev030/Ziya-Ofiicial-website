...
import course_highlight from "../../assets/Icons/Course/course_highlight.svg";

const SkillCoursePage = () => {
  const { courseId } = useParams();
  const course = mockCourses.find((c) => c.id === courseId);
  const [firstPart, secondPart] = splitParagraph(
    course?.aboutCoursePara2 ?? ""
  );
      ...
      <div className="w-full flex justify-center bg-white font-[Poppins]">
        <div className="max-w-[1445px] px-4  sm:px-6 lg:px-6 mt-10">
         ...
         object-cover   rounded-xl  border border-[#00A0E3]  mb-6  "
          ...
          {/* === Syllabus Section === */}
          <div className=" mx-auto  sm:px-1 lg:px-0 mt-6 flex flex-row items-center  gap-4">
            <img
              src={syllabus}
              alt="syllabus"
            ...
            {/* === Heading === */}
            <h2 className="font-poppins font-medium text-[18px] sm:text-[18px] md:text-[22px] lg:text-[24px] xl:text-[26px] text-gray-900 underline leading-[100%] tracking-[0%]">
              Syllabus
            </h2>
          </div>
          ...
          {course.groups?.map((group, index) => (
            ...
            >
              {/* === Number Circle === */}
              <div className="flex-shrink-0 bg-[#94C21A] rounded-full  flex items-center justify-center   text-white font-[Poppins]  w-[30px] h-[30px]  lg:w-[50px] lg:h-[50px]  md:w-[45px] md:h-[45px]  sm:w-[40px] sm:h-[40px]xs:w-[38px] xs:h-[38px]   ">
                <span className=" text-[0.8rem]lg:text-[1.1rem]  md:text-[1rem]  sm:text-[0.95rem]  xs:text-[0.9rem]  font-medium">
                  {index + 1}
                </span>
              </div>
              ...
              <div className="flex-1">
                <h3 className="  font-[Poppins] text-gray-900 font-semibold  text-[0.9rem]  lg:text-[1.25rem]  md:text-[1.1rem]  sm:text-[1rem]  xs:text-[0.95rem]  leading-[135%] sm:leading-[130%] xs:leading-[125%]  tracking-normal text-left     ">
                  {group.title}
                </h3>

                <p className="  font-[Poppins] text-gray-700 font-normal  text-[0.8rem]  lg:text-[1.125rem]  md:text-[1rem]  sm:text-[0.95rem]  xs:text-[0.9rem]  leading-[145%] sm:leading-[140%] xs:leading-[135%]  mt-2 sm:mt-1  tracking-normal text-left">
                  {group.description}
                </p>
              </div>
            </div>
          ...
          <div className=" mx-auto  sm:px-1 lg:px-0 mt-6 flex flex-row items-center  gap-4">
            <img
              src={course_highlight}
              alt="course_highlight"
              className="w-[30px] h-[30px] sm:w-[40px] sm:h-[40px] md:w-[50px] md:h-[50px] lg:w-[60px] lg:h-[60px] "
            />
          ...
          {course.courseHighlight?.map((course, index) => (
            <div
              key={index}
              className="mt-6 flex items-start gap-4 sm:gap-3 flex-wrap sm:flex-nowrap "
            >
